...
# Global model status tracking / Globaler Modell-Status
# Globale Status-Verfolgung für Modell-Laden und Downloads
_STATUS_LOCK = threading.Lock()
_STATUS = {
    "state": "idle",        # idle | downloading | loading | inferring
    "model": "",            # name of the model being processed
    "progress": -1.0,       # download progress 0-100, -1 if unknown
    "message": "",          # human-readable status message
}


def _set_status(state: str, model: str = "", progress: float = -1.0, message: str = "") -> None:
    """Set the current model status. / Aktuellen Modell-Status setzen."""
    with _STATUS_LOCK:
        _STATUS["state"] = state
        _STATUS["model"] = model
...
def _get_status() -> dict:
    """Get a copy of the current model status. / Kopie des aktuellen Modell-Status abrufen."""
    with _STATUS_LOCK:
        return dict(_STATUS)


# Intercept tqdm to track download progress
# tqdm abfangen um Download-Fortschritt zu verfolgen
import tqdm as _tqdm_module

_OriginalTqdm = _tqdm_module.tqdm


class _StatusTrackingTqdm(_OriginalTqdm):
    """Wraps tqdm to report download progress to the status endpoint.
    Wrapped tqdm, das den Download-Fortschritt an den Status-Endpunkt meldet."""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.total and self.total > 1024 * 1024:  # Only track downloads > 1MB
            _set_status(
                "downloading",
                model=_STATUS.get("model", ""),
                progress=0.0,
                message=f"Downloading: {self.desc or 'model files'}"
            )
...
_tqdm_module.tqdm = _StatusTrackingTqdm
# Also patch tqdm.auto which huggingface_hub often uses
# Auch tqdm.auto patchen, da huggingface_hub es oft verwendet
try:
    import tqdm.auto
    tqdm.auto.tqdm = _StatusTrackingTqdm
except ImportError:
    pass
...
def interrogate_image(network_name, data_object, data_type, net_params, skip_online):
    ...
    with INTERROGATOR_LOCK:
        ...
        # Pruefen ob Modell wirklich geladen werden muss
        # Check if model actually needs to be loaded
        # Wrapper haben .interrogator.model, Direktklassen haben .model
        # Wrappers have .interrogator.model, direct classes have .model
        inner = getattr(intg, 'interrogator', intg)
        inner = getattr(inner, 'tagger_inst', inner)
        needs_load = getattr(inner, 'model', None) is None or ACTIVE_INTERROGATOR != network_name
        ACTIVE_INTERROGATOR = network_name

        if needs_load:
            _set_status("loading", model=network_name, message=f"Loading model: {network_name}")
        intg.start(net_params, skip_online=skip_online)

        # Status: Inferenz laeuft / Status: Inference running
        _set_status("inferring", model=network_name, message=f"Processing image with {network_name}")
        tags = intg.predict(data_object, data_type)

        # Status: Fertig / Status: Done
        _set_status("idle", model=network_name, message="")
...
def edit_image(network_name, image_obj, net_params, skip_online):
    ...
    with INTERROGATOR_LOCK:
        ...
        inner = getattr(editor, 'editor', editor)
        needs_load = getattr(inner, 'model', None) is None or ACTIVE_INTERROGATOR != network_name
        ACTIVE_INTERROGATOR = network_name
        ...
        _set_status("inferring", model=network_name, message=f"Editing image with {network_name}")
...
def translate_text(network_name, text, from_lang, to_lang, net_params, skip_online):
    ...
    with INTERROGATOR_LOCK:
        ...
        inner = getattr(translator, 'translator', translator)
        needs_load = getattr(inner, 'model', None) is None or ACTIVE_INTERROGATOR != network_name
        ACTIVE_INTERROGATOR = network_name
        ...
        _set_status("inferring", model=network_name, message=f"Translating text with {network_name}")
...
def taggers_params(model_name):
    ...
    if int_instance.type == "wd":
        ...
        result.Parameters.append(
            create_interrogator_parameter("split", "false", "bool", "Split lines with commas"))
    elif int_instance.type == "llamacpp":
        result.Type = "llamacpp"
        result.Parameters.append(create_interrogator_parameter("query", int_instance.defaultPrompt, "string", ""))
        result.Parameters.append(create_interrogator_parameter("max_new_tokens", int_instance.max_new_tokens, "int",
                                                               "Maximum tokens to generate. Reasoning models need headroom; 512 is a good default."))
        result.Parameters.append(create_interrogator_parameter("n_gpu_layers", int_instance.n_gpu_layers, "int",
                                                               "GPU layers to offload (llama.cpp). 99 = all (needs a CUDA/Vulkan build); 0 = CPU only."))
        result.Parameters.append(
            create_interrogator_parameter("split", str(int_instance.split).lower(), "bool", "Split lines with commas"))
        result.Parameters.append(create_interrogator_parameter("Comment",
                                                               "GGUF vision model via bundled llama-server (llama.cpp).\n"
                                                               "First use downloads the model (several GB).",
                                                               "label", ""))
...
# Status endpoint for model loading/download progress
# Status-Endpunkt für Modell-Lade-/Download-Fortschritt
class GetStatus(Resource):
    def get(self):
        return _get_status(), 200
...
if __name__ == '__main__':
    models.init()
    host = os.environ.get("TAGMANAGER_SERVER_HOST", "127.0.0.1")
    port = int(os.environ.get("TAGMANAGER_SERVER_PORT", "50051"))
