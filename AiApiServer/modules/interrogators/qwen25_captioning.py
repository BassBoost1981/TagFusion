# pylint: disable=bad-indentation
import os

import torch
from PIL import Image
from transformers import Qwen2_5_VLForConditionalGeneration, AutoTokenizer, AutoProcessor
from qwen_vl_utils import process_vision_info
from transformers.video_utils import load_video

from .. import settings
from .. import devices as devices
from .. import utilities, paths
from ..server_dataclasses import ObjectDataType


class Qwen25CaptionCaptioning:
    def __init__(self, model_name):
        self.MODEL_REPO = model_name
        self.model = None
        self.processor = None
        # self.cmd = None
        self.prompt = None
        self.split = False
        self.fps = 1
        self.max_frames = -1
        self.min_pixels = -1
        self.max_pixels = -1

    def load(self, prompt, split, fps, max_frames, min_pixels, max_pixels, skip_online: bool = False):
        # self.cmd = cmd
        self.prompt = prompt
        self.split = split
        self.fps = fps
        self.max_frames = max_frames
        self.min_pixels = min_pixels
        self.max_pixels = max_pixels
        if self.model is None or self.processor is None:
            # Windows-compatible memory optimization WITHOUT bitsandbytes
            # Uses float16/bfloat16 instead (50% VRAM reduction)
            
            # Determine best dtype for your GPU
            # bfloat16 is better for RTX 30xx/40xx (Ampere/Ada)
            # float16 is better for older GPUs
            try:
                # Try bfloat16 first (better quality, same memory as float16)
                dtype = torch.bfloat16
            except:
                # Fallback to float16 if bfloat16 not supported
                dtype = torch.float16
            
            self.model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
                self.MODEL_REPO,
                torch_dtype=dtype,  # Use half precision (50% VRAM reduction)
                cache_dir=paths.setting_model_path,
                local_files_only=skip_online,
                device_map="auto",
                # Windows-compatible optimizations:
                low_cpu_mem_usage=True,  # Reduce CPU RAM usage during loading
            )
            
            # Enable gradient checkpointing for additional memory savings
            if hasattr(self.model, 'gradient_checkpointing_enable'):
                self.model.gradient_checkpointing_enable()

            self.processor = AutoProcessor.from_pretrained(
                self.MODEL_REPO,
                trust_remote_code=True,
                cache_dir=paths.setting_model_path,
                local_files_only=skip_online
            )

    def unload(self):
        if not settings.current.interrogator_keep_in_memory:
            self.model = None
            self.processor = None
            devices.torch_gc()

    def apply(self, data_obj, data_type: ObjectDataType):
        if self.model is None or self.processor is None:
            return ""
        messages = None
        if data_type == ObjectDataType.IMAGE_BYTE_ARRAY:
            image = utilities.byte_array_to_image(data_obj)
            messages = [
                {
                    "role": "user",
                    "content": [
                        {
                            "type": "image",
                            "image": image,
                        },
                        {"type": "text", "text": self.prompt},
                    ],
                },
            ]
            # CRITICAL: Limit image resolution to reduce VRAM usage
            # Default Qwen processes images at very high resolution
            if self.min_pixels == -1:
                # Set reasonable default if not specified
                messages[0]["content"][0]["min_pixels"] = 224 * 224  # Minimum resolution
            else:
                messages[0]["content"][0]["min_pixels"] = self.min_pixels
                
            if self.max_pixels == -1:
                # IMPORTANT: Limit max pixels to reduce VRAM
                # Default is often 1280*28*28 = ~45M pixels which uses tons of VRAM
                # Reducing to 768*768 = ~590K pixels saves significant VRAM
                messages[0]["content"][0]["max_pixels"] = 768 * 768  # Reduced from default
            else:
                messages[0]["content"][0]["max_pixels"] = self.max_pixels
                
        elif data_type == ObjectDataType.VIDEO_PATH:
            video_path = data_obj.decode("utf-8")
            messages = [
                {
                    "role": "user",
                    "content": [
                        {
                            "type": "video",
                            "video": video_path,
                            "fps": self.fps,
                        },
                        {"type": "text", "text": self.prompt},
                    ],
                }
            ]
            if self.max_frames != -1:
                messages[0]["content"][0]["max_frames"] = self.max_frames
            if self.min_pixels != -1:
                messages[0]["content"][0]["min_pixels"] = self.min_pixels
            if self.max_pixels != -1:
                messages[0]["content"][0]["max_pixels"] = self.max_pixels

        if settings.current.custom_system_prompt != "":
            messages.insert(0, {"role": "system", "content": settings.current.custom_system_prompt})
        
        # Preparation for inference
        text = self.processor.apply_chat_template(
            messages, tokenize=False, add_generation_prompt=True
        )
        
        # FIXED: Removed unsupported image_patch_size parameter
        image_inputs, video_inputs = process_vision_info(messages)
        
        inputs = self.processor(
            text=[text],
            images=image_inputs,
            videos=video_inputs,
            padding=True,
            return_tensors="pt",
        )
        inputs = inputs.to(devices.device)

        # Inference with memory optimization
        with torch.no_grad():  # Disable gradients (we're not training)
            # Use autocast for automatic mixed precision
            with torch.cuda.amp.autocast():
                generated_ids = self.model.generate(**inputs, max_new_tokens=128)
        
        generated_ids_trimmed = [
            out_ids[len(in_ids):] for in_ids, out_ids in zip(inputs.input_ids, generated_ids)
        ]
        output_text = self.processor.batch_decode(
            generated_ids_trimmed, skip_special_tokens=True, clean_up_tokenization_spaces=False
        )
        
        # IMPORTANT: Clear CUDA cache after each inference
        # This prevents memory fragmentation
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        
        if self.split:
            return [x.strip() for x in output_text[0].split(',')]
        return output_text
